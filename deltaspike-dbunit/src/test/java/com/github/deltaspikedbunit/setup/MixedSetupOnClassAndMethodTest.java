...
public class MixedSetupOnClassAndMethodTest {
	...
	@Test
	@DatabaseSetup(value = "../extra_category_with_items.xml", type = DatabaseOperation.INSERT)
	public void test() throws Exception {
		List<String> names = inventoryRepository.findAllNames();
		Assert.assertEquals(3, names.size());
		assertThat(names, contains("existing_category", "default_category", "extra_category"));
	}

	@Test
	@DatabaseSetup(value = "../refresh-db.xml", type = DatabaseOperation.REFRESH)
	public void testAgain() throws Exception {
		List<String> names = inventoryRepository.findAllNames();
		Assert.assertEquals(3, names.size());
		assertThat(names, contains("existing_category", "default_category", "refreshed_category"));
	}
